...
Import as a side effect before importing from wos:

    import _bootstrap  # noqa: F401

When run as ``python scripts/<name>.py``, Python adds ``scripts/`` to
sys.path[0], so ``import _bootstrap`` finds this file. It resolves the
plugin root and inserts it so that ``import wos`` succeeds.
